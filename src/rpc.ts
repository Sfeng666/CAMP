...
function endpointDescriptor(): string {
...
type ConnectionTarget =
  ...
  | { kind: "file"; requests: string; responses: string };

function connectionTarget(): ConnectionTarget {
...
export function ensureRpcToken(): string {
  ...
  if (!/^[a-f0-9]{64}$/i.test(token)) throw new Error(`Invalid CAMP RPC token: ${paths.rpcToken}`);
  return token;
}

export function defaultRpcIdentity(
  deliveryMode: RpcIdentity["deliveryMode"] = "cli",
): RpcIdentity {
  return {
...
function daemonOwnerPid(): number | null {
...
export async function waitForDaemonExit(timeoutMs = 120_000): Promise<void> {
...
function invocation(): { command: string; args: string[] } {
  ...
  if (current) return { command: process.execPath, args: [current, "daemon"] };
...
async function rawCall(
  ...
  const id = randomUUID();
  const payload = `${JSON.stringify({ id, token: ensureRpcToken(), method, params, client } satisfies RpcRequest)}\n`;
  if (Buffer.byteLength(payload) > MAX_FRAME) throw new Error("CAMP RPC request exceeds the 4 MiB limit");
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on("data", (chunk: string) => {
      ...
      try {
        ...
        if (response.id !== id) throw new Error("CAMP daemon returned a mismatched RPC response");
        if (response.error) {
          const error = new Error(response.error.message) as Error & { code?: string };
          if (response.error.code) error.code = response.error.code;
          reject(error);
...
export async function rpcCall<T = unknown>(
  ...
  return (await rawCall(method, params, client, timeoutMs)) as T;
}

export async function ensureDaemon(startupWaitMs = 5_000): Promise<void> {
  ensureRpcToken();
  try {
...
export async function callDaemon<T = unknown>(
  ...
  if (!daemonOwnerPid()) await ensureDaemon();
  return rpcCall<T>(method, params, client, timeoutMs);
}
...
function fileRpcServer(
  handler: (method: string, params: Record<string, unknown>, client: RpcIdentity) => Promise<unknown>,
  token: string,
  descriptor: string,
): RpcServerHandle {
    ...
    for (const name of names) {
      ...
      void (async () => {
        ...
        try {
          ...
          if (Buffer.byteLength(raw) > MAX_FRAME) throw Object.assign(new Error("RPC frame too large"), { code: "FRAME_TOO_LARGE" });
          request = JSON.parse(raw) as RpcRequest;
          if (request.token !== token) throw Object.assign(new Error("CAMP RPC authentication failed"), { code: "UNAUTHORIZED" });
          const result = await handler(request.method, request.params ?? {}, request.client);
          atomicWrite(join(responses, name), `${JSON.stringify({ id: request.id, result } satisfies RpcResponse)}\n`, 0o600);
        } catch (error) {
    ...
    close(callback) {
      ...
      Promise.all(
        [...active].map(async (name) => {
          while (active.has(name)) await new Promise((resolve) => setTimeout(resolve, 5));
        }),
...
export async function startRpcServer(
  ...
  if (hostPlatform() !== "windows" && existsSync(endpoint)) unlinkSync(endpoint);
  ...
  try {
    ...
    if (hostPlatform() !== "windows") ensurePrivateFile(endpoint);
  } catch (error) {
    ...
    try {
      ...
      if (!new Set(["EPERM", "EACCES", "EOPNOTSUPP"]).has(String(fallbackCode))) throw fallbackError;
      return fileRpcServer(handler, token, descriptor);
    }
    const address = server.address();
    if (!address || typeof address === "string") throw new Error("CAMP RPC loopback fallback did not expose a port");
    atomicWrite(descriptor, `${JSON.stringify({ host: "127.0.0.1", port: address.port })}\n`, 0o600);
  }
  return server;
}
