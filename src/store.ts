...
export class CampStore {
  ...
  private storedSession(sessionId: string): StoredSessionSummary | undefined {
    return this.db
      .prepare("SELECT content_hash, archive_path, message_count FROM sessions WHERE id = ?")
      .get(sessionId) as StoredSessionSummary | undefined;
  }

  private sessionPrefixMatches(
    sessionId: string,
    messages: CanonicalSession["messages"],
    count: number,
  ): boolean {
    if (count < 0 || count > messages.length) return false;
  ...
  private commitSession(
    ...
    contentHash: string,
    archivePath: string,
    existing: StoredSessionSummary | undefined,
  ): StoreSessionResult {
  ...
  async storeSessionAsync(
    ...
    const archivePath = join(archiveRoot, `${contentHash}.json.gz`);
    if (!existsSync(archivePath)) {
      const compressed = await cooperativeAwait(
        gzipAsync(Buffer.from(JSON.stringify(session), "utf8")),
        cooperate,
      );
  ...
  listSessionIdsSince(projectId: string, importedAfter: string | null): string[] {
    const rows = this.db
      ...
      `)
      .all(...(importedAfter ? [projectId, importedAfter] : [projectId])) as Array<{ id: string }>;
    return rows.map((row) => row.id);
  }
  ...
  sessionArchiveInfo(
    sessionId: string,
    projectId: string,
  ): { archivePath: string; sourcePath: string; messageCount: number } | null {
    const row = this.db
      .prepare(`
  ...
  *semanticVectors(
    ...
    const layers: Array<{ layer: "raw" | "curated"; budget: number }> = source === "all"
      ? [
    ...
    for (const { layer, budget } of layers) {
      ...
      const rows = this.db
        ...
        `)
        .iterate(projectId, modelDigest, layer, budget) as Iterable<{
        layer: "raw" | "curated";
        document_id: string;
        vector_json: string;
      }>;
  ...
  resolveQuarantine(id: string, projectId: string): boolean {
    const resolve = this.db.transaction(() => {
      const row = this.db
        ...
        .get(id) as { source: AgentSource; source_path: string; native_id: string | null } | undefined;
      if (!row) return false;
      ...
      this.db
        ...
        .run(
          projectId,
          row.source,
          stableId(row.source_path),
          stableId(row.source_path, row.native_id),
        );
      return true;
    });
    return resolve();
  ...
  recoverInterruptedSyncs(): number {
    const rows = this.db
      ...
      .all() as Array<{ id: string; project_id: string; source: AgentSource }>;
    ...
    const transaction = this.db.transaction(() => {
      const finish = this.db.prepare(
        "UPDATE sync_runs SET completed_at=?, status='degraded', error_json=? WHERE id=?",
      );
    ...
    const transaction = this.db.transaction(() => {
      ...
      this.db
        ...
        .run(
          ...
          summary.skipped,
          summary.quarantined,
          error ? JSON.stringify(error) : degraded ? JSON.stringify({ message: summary.errors[0] }) : null,
        );
    });
    transaction();
  }
  ...
  sourceFreshness(projectId: string): SourceFreshness[] {
    ...
    return rows.map((row) => {
      ...
      return {
        ...
        status: row.status as SourceFreshness["status"],
        ...
        error: typeof row.error_json === "string"
          ? json<ImportErrorDetail | null>(row.error_json, null)
          : null,
      };
    });
  }
  ...
  contextReceipt(id: string): {
    ...
    const row = this.db
      ...
      .get(id) as { payload_json: string; challenge_hash: string; acknowledged_at: string | null } | undefined;
    if (!row) return null;
    return {
      receipt: json<ContextReceipt>(row.payload_json, {} as ContextReceipt),
  ...
  contextAcknowledgment(receiptId: string): ContextAcknowledgment | null {
    ...
    return {
      ...
      projectId: String(row.project_id),
      client: json<ContextAcknowledgment["client"]>(String(row.client_json), {
        name: "unknown",
        version: "unknown",
        instanceId: "unknown",
      }),
      evidenceIds: json<string[]>(String(row.evidence_ids_json), []),
      recalledFact: String(row.recalled_fact),
      acknowledgedAt: String(row.acknowledged_at),
      verdict: row.verdict as ContextAcknowledgment["verdict"],
      reasons: json<string[]>(String(row.reasons_json), []),
    };
  }
    ...
    const transaction = this.db.transaction(() => {
      this.db
        ...
        .run(
          ...
          acknowledgment.projectId,
          JSON.stringify(acknowledgment.client),
          JSON.stringify(acknowledgment.evidenceIds),
          acknowledgment.recalledFact,
          acknowledgment.verdict,
          JSON.stringify(acknowledgment.reasons),
  ...
  createVerificationRun(input: {
    ...
    this.db
      ...
      .run(
        ...
        run.sourceAgent,
        run.sourceSurface,
        JSON.stringify(run.sourceClient),
        JSON.stringify(run.targetAgents),
        run.canary,
        run.canaryHash,
        run.evidenceId,
        run.status,
  ...
  verificationRun(id: string): VerificationRun | null {
    const row = this.db.prepare("SELECT * FROM verification_runs WHERE id=?").get(id) as
      | Record<string, unknown>
      | undefined;
    if (!row) return null;
    let status = String(row.status) as VerificationRun["status"];
    ...
    if (!row.cancelled_at && expired && !alreadyScrubbed) {
      if (status !== "passed") status = "expired";
      const scrubbed = `Expired CAMP verification ${String(row.canary_hash).slice(0, 16)}`;
      const transaction = this.db.transaction(() => {
        this.db.prepare("UPDATE verification_runs SET status=?, canary=? WHERE id=?").run(
          status,
          `expired:${String(row.canary_hash).slice(0, 16)}`,
          id,
        );
        this.db
    ...
    return {
      ...
      id: String(row.id),
      projectId: String(row.project_id),
      sourceAgent: row.source_agent as AgentSource,
      sourceSurface: row.source_surface as VerificationRun["sourceSurface"],
      sourceClient: json<VerificationRun["sourceClient"]>(String(row.source_client_json), {
        name: "unknown",
        version: "unknown",
        instanceId: "unknown",
      }),
      targetAgents: json<AgentSource[]>(String(row.target_agents_json), []),
  ...
  refreshVerificationRun(id: string): VerificationRun | null {
    ...
    if (!run || run.status === "cancelled" || run.status === "expired") return run;
    if (!run.rawSessionId && run.canary) {
      const rows = this.db
        ...
        `)
        .all(run.projectId, run.sourceAgent, run.canary, run.createdAt, run.createdAt) as Array<{
          session_id: string;
        }>;
  ...
  verificationSearchHits(run: VerificationRun): SearchHit[] {
    ...
    if (run.rawSessionId && run.canary) {
      const row = this.db
        ...
        `)
        .get(run.projectId, run.rawSessionId, run.canary) as Record<string, unknown> | undefined;
      if (row) {
        hits.push({
          layer: "raw",
          id: String(row.id),
  ...
  cancelVerificationRun(id: string): VerificationRun | null {
    ...
    const run = this.verificationRun(id);
    if (!run) return null;
    if (run.status === "passed" || run.status === "cancelled" || run.status === "expired") return run;
