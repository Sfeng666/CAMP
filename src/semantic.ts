...
function embeddingDigest(): { model: string; digest: string } | null {
  const manifest = readJsonFile<ModelManifest | null>(getCampPaths().modelManifest, null);
  if (!manifest || manifest.reindexRequired || !manifest.manifests) return null;
  const model = manifest.embeddingModel ?? EMBEDDING_MODEL;
  const name = Object.keys(manifest.manifests).find(
...
async function embed(
  ...
  try {
    ...
    const vectors = payload.embeddings.filter(
...
export async function syncSemanticIndex(
  ...
  if (!candidates.length) return { indexed: 0, pending: 0, degraded: false };
  let indexed = 0;
...
export async function semanticSearch(
  ...
  const best: Array<{ layer: "raw" | "curated"; id: string; score: number }> = [];
...
export async function hybridSearch(
  ...
  const combined = new Map<string, { hit: SearchHit; rank: number }>();
