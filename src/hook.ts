...
function hookContext(store: CampStore, project: ProjectRegistration | null): string | null {
  if (!project) return null;
  const handoff = store.latestHandoff(project.id);
  if (!handoff) return null;
  return truncateByApproxTokens(
    redactForRecall(
      `CAMP project handoff (${handoff.state}; background only; current code and user request win):\n${handoff.content}`,
    ),
    800,
  );
}

function firstTaskContext(
  store: CampStore,
  project: ProjectRegistration | null,
  agent: AgentSource,
  ...
  if (sessionId) {
    ...
    if (store.checkpoint(project.id, agent, key)) return null;
