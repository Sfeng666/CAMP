...
function legacyWriterPid(): number | null {
...
function acquireMigrationLock(path: string): number {
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (alive(owner)) {
        throw Object.assign(
          new Error(`CAMP schema migration is already owned by PID ${owner}`),
...
export async function bootstrapDatabase(): Promise<{ migrated: boolean; backup: string | null }> {
  const paths = ensureCampDirectories(getCampPaths());
  let current = schemaVersion(paths.database);
  if (current >= SCHEMA_VERSION) return { migrated: false, backup: null };
  ...
  try {
    ...
    if (writer) {
      throw new Error(
        `CAMP daemon PID ${writer} still owns the pre-v${SCHEMA_VERSION} database; run camp upgrade --apply`,
      );
    }
    ...
    closeSync(lock);
    if (existsSync(paths.migrationLock)) unlinkSync(paths.migrationLock);
  }
}
