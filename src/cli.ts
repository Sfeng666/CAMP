...
import type { AgentSource, EvidenceKind, EvidenceState, HandoffInput, ImportSummary } from "./types.js";
import { inspectProject, changedPaths, currentCommit, worktreeFingerprint } from "./git.js";
...
function requireSupportedNode(): void {
  ...
  for (let index = 0; index < MINIMUM_NODE.length; index += 1) {
    ...
    if (actual < required) {
      throw new Error(
        `CAMP requires Node.js 22.18.0 or newer; current runtime is ${process.versions.node}`,
      );
    }
  }
}
...
function parseAgent(value: string): AgentSource {
  const allowed = new Set<AgentSource>([
    "codex",
    "claude",
    "cursor",
    "antigravity",
...
function startBackgroundSync(projectId: string, cwd: string): boolean {
      ...
      if (options.import) {
        if (process.env.CAMP_SETUP_FOREGROUND === "1") {
          ...
          if (result.chatcrystal) {
            line(
              `chatcrystal: imported=${result.chatcrystal.imported} replaced=${result.chatcrystal.replaced} skipped=${result.chatcrystal.skipped} errors=${result.chatcrystal.errors}`,
            );
          }
          for (const error of result.errors) line(`degraded: ${error}`);
...
program
  ...
  .option("--json", "Emit JSON")
  .action((path: string, options: { assign?: string; json?: boolean }) => {
    const store = new CampStore();
    try {
      const project = resolveProject(store, path);
      if (options.assign) {
        const resolved = store.resolveQuarantine(options.assign, project.id);
        if (!resolved) throw new Error(`Open quarantine item not found: ${options.assign}`);
      }
  ...
  .action(async (query: string, options: { project: string; source: string; limit: string; json?: boolean }) => {
    ...
    try {
      ...
      const source = new Set(["raw", "curated", "all"]).has(options.source)
        ? (options.source as "raw" | "curated" | "all")
        : "all";
...
program
  ...
  .action((path: string, options: {
    ...
    try {
      ...
      if (options.json) json({ record, memorix: backend });
      else line(`Created handoff ${record.id}; Memorix completed=${backend.completed} failed=${backend.failed}`);
    } finally {
      store.close();
    }
  });
  ...
  .action(async (path: string, options: { purge?: boolean; confirm?: string }) => {
    ...
    try {
      ...
      if (options.purge) {
        ...
        line(
          `memorix: deleted=${memorix.deleted} already_deleted=${memorix.alreadyDeleted}`,
        );
...
program
  ...
  .action((options: { embeddingDigest: string }) => {
    const store = new CampStore();
    try {
