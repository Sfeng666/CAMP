...
function evidenceKind(value: unknown): EvidenceKind {
  ...
  if (!allowed.has(value as EvidenceKind)) throw new Error(`Invalid evidence kind: ${String(value)}`);
  return value as EvidenceKind;
}

function evidenceState(value: unknown): EvidenceState {
  const allowed = new Set<EvidenceState>(["candidate", "verified", "stale"]);
  if (!allowed.has(value as EvidenceState)) throw new Error(`Invalid evidence state: ${String(value)}`);
  return value as EvidenceState;
}

function agent(value: unknown): AgentSource {
  const allowed = new Set<AgentSource>(["codex", "claude", "cursor", "antigravity", "archive", "unknown"]);
  return allowed.has(value as AgentSource) ? (value as AgentSource) : "unknown";
}

function memorixQueueState(store: CampStore, projectId: string): { queued: boolean; pending: number } {
...
export class CampService {
  constructor(
    ...
    private readonly requestFastSync: (projectId?: string) => void = () => undefined,
  ) {}
  ...
  private async makeFresh(projectId: string, verificationRunId?: string | null): Promise<void> {
  ...
  async call(method: string, params: Record<string, unknown>, client: RpcIdentity): Promise<unknown> {
    switch (method) {
      case "ping":
      ...
      case "search": {
        ...
        const source = new Set(["raw", "curated", "all"]).has(string(params.source))
          ? (string(params.source) as "raw" | "curated" | "all")
          : "all";
        return hybridSearch(
          this.store,
          project.id,
          string(params.query),
          source,
          Math.max(1, Math.min(50, number(params.limit, 20))),
        );
      }
      case "conversation": {
        const project = this.project(params.project);
        const session = this.store.getSession(string(params.conversationId), project.id);
        if (!session) throw new Error(`Conversation not found in this project: ${String(params.conversationId)}`);
        return session;
      }
      case "recordMemory": {
        const project = this.project(params.project);
        const kind = evidenceKind(params.kind);
        if (kind === "handoff") throw new Error("Use createHandoff for handoff records");
      ...
      case "remove": {
        ...
        if (purge) {
          ...
          if (existsSync(archive)) rmSync(archive, { recursive: true, force: false });
        }
      ...
      case "capture": {
        ...
        const payload = params.payload && typeof params.payload === "object"
          ...
          : {};
        const output = captureHook(
          this.store,
          source,
          event,
          payload,
          (projectId) => this.requestFastSync(projectId),
        );
        const receiptEvent =
        ...
        try {
          ...
          const unavailable =
            `CAMP context receipt could not be issued (${message}). ` +
