...
import { callDaemon, type RpcIdentity } from "./rpc.js";
import { CAMP_VERSION } from "./version.js";

const MCP_INSTRUCTIONS =
  "CAMP supplies project-scoped local memory. At session/task start, call camp_context_for_task, then acknowledge the exact receipt through camp_ack_context before saying context is verified. A PASS proves delivery and evidence identification, not private reasoning. Current code, Git state, and explicit project rules outrank memory. Never use cross-project results unless the user explicitly requests them.";
...
async function writeMcpMessage(message: Record<string, unknown>): Promise<void> {
  const output = `${JSON.stringify(message)}\n`;
  if (!process.stdout.write(output)) await once(process.stdout, "drain");
}

async function writeMcpResult(id: JsonRpcId, resultValue: unknown): Promise<void> {
  await writeMcpMessage({ jsonrpc: "2.0", id, result: resultValue });
}

async function writeMcpError(
  id: JsonRpcId,
  code: number,
  message: string,
  data?: unknown,
...
function parseClientVersion(params: Record<string, unknown>): McpClientVersion | null {
...
async function serveMcp(
  ...
  callTool: (name: string, argumentsValue: unknown) => Promise<Record<string, unknown>>,
  setClientVersion: (client: McpClientVersion | null) => void,
): Promise<void> {
  ...
  for await (const chunk of process.stdin) {
    ...
    for (;;) {
      ...
      try {
        ...
        request = candidate as unknown as JsonRpcRequest;
      } catch {
      ...
      try {
        switch (request.method) {
          ...
          case "notifications/cancelled":
            break;
          case "ping":
            if (!notification) await writeMcpResult(request.id ?? null, {});
            break;
          case "tools/list":
            if (!notification) await writeMcpResult(request.id ?? null, { tools });
            break;
          case "tools/call": {
            if (notification) break;
  ...
  if (buffer.trim()) await writeMcpError(null, -32700, "Incomplete JSON-RPC frame");
}
...
function result(value: unknown, humanText?: string): Record<string, unknown> {
  return {
    content: [
      {
        type: "text",
...
function contextResult(response: { text: string; receipt: Record<string, unknown> }): Record<string, unknown> {
...
function failure(value: unknown): Record<string, unknown> {
  return {
...
export async function runMcpServer(): Promise<void> {
  ...
  let clientVersion: McpClientVersion | null = null;
  const identity = (): RpcIdentity => {
    ...
    const surfaceHint =
      configuredSurface === "cli" || configuredSurface === "ide" || configuredSurface === "desktop"
        ? configuredSurface
  ...
  const tools: Array<Record<string, unknown>> = [
      {
        ...
        description:
          "Retrieve bounded current project context and a signed receipt. Call camp_ack_context before claiming CAMP context is verified.",
        annotations: {
          readOnlyHint: false,
      ...
      },
      {
        ...
        description:
          "Acknowledge the exact signed CAMP context receipt from this MCP client instance. Copy receipt.evidenceIds exactly; added or substituted IDs fail closed. Returns PASS, WARN, or FAIL.",
        annotations: {
          readOnlyHint: false,
          destructiveHint: false,
          idempotentHint: false,
      ...
      },
      {
        ...
        description:
          "Start an expiring project-isolated canary from the current agent. Echo the returned canary exactly so CAMP can prove transcript import.",
        annotations: {
          readOnlyHint: false,
          destructiveHint: false,
          idempotentHint: false,
    ...
    try {
      switch (name) {
        case "camp_context_for_task": {
          const response = await callDaemon<{ text: string; receipt: Record<string, unknown> }>(
            "context",
            {
              task: args.task,
