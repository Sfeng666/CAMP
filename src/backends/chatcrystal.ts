...
const emptyResult = (): ChatCrystalResult => ({
  total: 0,
  imported: 0,
  replaced: 0,
  skipped: 0,
...
function workerInvocation(mode: "ingest" | "purge", projectId?: string): {
  command: string;
  args: string[];
} {
...
async function runWorker(input: {
  ...
  try {
    ...
    if (status !== 0) {
      throw new Error(
        redactForRecall(stderr.trim() || `ChatCrystal worker exited ${status}`).slice(0, 64 * 1024),
      );
    }
...
export async function syncChatCrystal(
  ...
  async function* records(): AsyncGenerator<string> {
    for (const id of ids) {
      ...
      if (!archive || archive.messageCount < 2 || !existsSync(archive.archivePath)) continue;
  ...
  const result = (await cooperativeAwait(
    runWorker({ mode: "ingest", dataDir, records: records() }),
    cooperate,
  )) as ChatCrystalResult;
  if (!result.errors) store.setCheckpoint(project.id, "archive", checkpointKey, nowIso());
  ensurePrivateFile(join(dataDir, "chatcrystal.db"));
  return result;
}
...
export async function purgeChatCrystalProject(
  ...
  const dataDir = join(store.paths.backendDir, "chatcrystal");
  ensurePrivateDirectory(dataDir);
  if (!existsSync(join(dataDir, "chatcrystal.db"))) return { deleted: 0 };
