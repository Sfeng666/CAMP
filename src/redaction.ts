...
export function redactForRecall(value: string): string {
  let result = value;
...
export function automaticMemoryExclusion(value: string): string | null {
  ...
  const outwardAsk =
    /\b(?:refer(?:ral| me)?|coffee chat|connect me|put me in touch|consider my application|hiring manager)\b/iu.test(
      value,
    );
  ...
  if (
    /\b(?:we (?:worked|studied|collaborated)|our time (?:at|together)|you may remember me from)\b/iu.test(
      value,
    )
  ) {
