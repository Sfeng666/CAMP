...
function databaseMentionsProject(path: string, project: ProjectRegistration): boolean {
  ...
  try {
    ...
    for (const { name } of tables) {
      const columns = db.prepare(`PRAGMA table_info(${quotedIdentifier(name)})`).all() as Array<{
        name: string;
      }>;
      for (const column of columns) {
        const sql = `SELECT 1 FROM ${quotedIdentifier(name)} WHERE instr(CAST(${quotedIdentifier(column.name)} AS TEXT), ?) > 0 LIMIT 1`;
        if (db.prepare(sql).get(project.rootPath)) return true;
      }
    }
  } catch {
    return false;
...
async function importHookSpool(
  store: CampStore,
  ...
  await readJsonLines(path, (entry, line) => {
    if (entry.projectId !== project.id || entry.agent !== "antigravity") return;
    const payload = entry.payload && typeof entry.payload === "object"
      ? (entry.payload as Record<string, unknown>)
      : {};
    const sessionId = String(
      entry.sessionId ?? payload.conversationId ?? payload.session_id ?? "antigravity-hook",
    );
    ...
    let kind: "message" | "tool-call" | "tool-result" | "event" = "event";
...
function transcriptWorkspacePaths(entry: Record<string, unknown>): string[] {
  const values = [
    ...
    entry.payload && typeof entry.payload === "object"
      ? (entry.payload as Record<string, unknown>).workspacePaths
      : null,
  ];
...
function transcriptMessage(
  ...
  const role = entry.role ?? payload.role ?? (
    /tool/.test(type) ? "tool" : /user|prompt|invocation/.test(type) ? "user" : /assistant|agent|response/.test(type) ? "assistant" : "unknown"
  );
...
async function importNativeTranscripts(
  store: CampStore,
  project: ProjectRegistration,
  summary: ImportSummary,
  ...
  for (const root of roots) {
    ...
    for (const path of transcripts) {
      ...
      await readJsonLines(path, (entry, sourceLine) => {
        for (const workspace of transcriptWorkspacePaths(entry)) paths.add(resolve(workspace));
        const payload = entry.payload && typeof entry.payload === "object"
          ? (entry.payload as Record<string, unknown>)
          : {};
        if (typeof entry.conversationId === "string") conversationId = entry.conversationId;
