...
export function normalizeClientAgent(name: string): AgentSource {
  const value = name.toLowerCase();
  if (value.includes("antigravity") || value.includes("gemini")) return "antigravity";
  if (value.includes("cursor")) return "cursor";
  if (value.includes("claude")) return "claude";
  if (value.includes("codex") || value.includes("chatgpt")) return "codex";
  return "unknown";
}

export function normalizeClientSurface(name: string, hint?: AgentSurface): AgentSurface {
  if (hint && hint !== "unknown") return hint;
  const value = name.toLowerCase();
  if (value.includes("antigravity")) return value.includes("cli") ? "cli" : "desktop";
  ...
  if (value.includes("cursor")) return value.includes("agent") || value.includes("cli") ? "cli" : "unknown";
  if (value.includes("claude") || value.includes("codex")) return "cli";
  return "unknown";
}

function secret(path: string): Buffer {
  if (!existsSync(path)) atomicWrite(path, `${randomBytes(32).toString("hex")}\n`, 0o600);
  const value = readFileSync(path, "utf8").trim();
  if (!/^[a-f0-9]{64}$/i.test(value)) throw new Error(`Invalid CAMP secret file: ${path}`);
  return Buffer.from(value, "hex");
}

function signedPayload(receipt: ContextReceipt, challengeHash: string): string {
...
function signReceipt(store: CampStore, receipt: ContextReceipt, challengeHash: string): string {
...
export async function issueContextReceipt(input: {
  ...
  const evidence = hits
    .map((hit) =>
      `- [${hit.id}] [${hit.layer}/${hit.source}${hit.state ? `/state=${hit.state}` : ""}] ${hit.title}\n  ${hit.content}\n  ${hit.uri}`,
    )
    .join("\n");
  ...
  let baseVerdict: ContextVerdict = "pending_ack";
  ...
  const receiptBase: ContextReceipt = {
    schemaVersion: 1,
    id: newId(),
...
export function acknowledgeContext(input: {
  ...
  if (!stored) throw new Error(`Unknown CAMP context receipt: ${input.receiptId}`);
  if (stored.acknowledgedAt) throw new Error(`Receipt was already acknowledged: ${input.receiptId}`);
  const receipt = stored.receipt;
  const reasons = [...receipt.reasons];
  let verdict: ContextAcknowledgment["verdict"] = receipt.verdict === "pending_ack" ? "PASS" : receipt.verdict;
  ...
  if (!project) {
    ...
    receipt.currentCommit !== currentCommit(project.rootPath) ||
  ...
  const acknowledgment: ContextAcknowledgment = {
    schemaVersion: 1,
    id: newId(),
...
export function contextStatus(store: CampStore, receiptId: string): Record<string, unknown> {
  const stored = store.contextReceipt(receiptId);
  if (!stored) throw new Error(`Unknown CAMP context receipt: ${receiptId}`);
  return {
...
export function startVerification(input: {
  ...
  const targets = [...new Set(input.targetAgents)].filter(
    (agent) => agent !== "unknown" && agent !== "archive" && agent !== sourceAgent,
  );
  if (!targets.length) throw new Error("At least one different target agent is required");
...
export function verificationStatus(store: CampStore, runId: string): Record<string, unknown> {
  const run = store.refreshVerificationRun(runId);
  if (!run) throw new Error(`Unknown CAMP verification run: ${runId}`);
  const acknowledgments = store.verificationAcknowledgments(run.id);
  const passedAgents = new Set(
    acknowledgments
