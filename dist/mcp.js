...
export async function runMcpServer() {
    ...
    const server = new Server({ name: "camp", version: CAMP_VERSION }, { capabilities: { tools: {} } });
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            {
                name: "camp_context_for_task",
                description: "Call once at the first substantive prompt in a registered project. Returns a bounded current handoff and task-specific prior evidence; current code and user instructions always win.",
                inputSchema: {
                    type: "object",
                    properties: {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            const project = resolveProject(store, typeof input.project === "string" ? input.project : process.cwd());
            ...
            switch (request.params.name) {
                ...
                case "camp_get_conversation": {
                    ...
                    if (!session)
                        throw new Error(`Conversation not found in this project: ${id}`);
                    return text(session);
                }
                case "camp_record_memory": {
                    const kind = String(input.kind);
                    const state = String(input.state ?? "candidate");
                    if (!evidenceKinds.has(kind) || kind === "handoff")
                    ...
                    const record = store.putEvidence({
                        ...
                        sourceAgent: new Set(["codex", "claude", "cursor", "antigravity", "unknown"]).has(String(input.source_agent))
                            ? String(input.source_agent)
                            : "camp",
                        sourceSessionId: typeof input.source_session_id === "string" ? input.source_session_id : null,
                        sourceUri: typeof input.source_session_id === "string"
