...
export class CampStore {
    db;
    paths = ensureCampDirectories(getCampPaths());
    databasePath;
    ...
    `);
        ...
        if (!messageColumns.some((column) => column.name === "content_hash")) {
            ...
            const rows = this.db.prepare("SELECT id, content FROM messages").all();
            const update = this.db.prepare("UPDATE messages SET content_hash=? WHERE id=?");
            const backfill = this.db.transaction(() => {
                for (const row of rows)
                    update.run(sha256(row.content), row.id);
            });
            backfill();
        }
    ...
    registerProject(inspected) {
        const strongAliases = inspected.aliases.filter((alias) => alias.confidence >= 0.9);
        const matched = new Set();
        const findAlias = this.db.prepare("SELECT project_id FROM project_aliases WHERE kind = ? AND value = ?");
        for (const alias of strongAliases) {
            const row = findAlias.get(alias.kind, alias.value);
            if (row)
        ...
        const migrationState = inspected.kind === "git" &&
        ...
        `)
                .run(id, inspected.kind, inspected.rootPath, inspected.filesystemId, inspected.gitRoot, inspected.gitCommonDir, inspected.rootCommit, JSON.stringify(inspected.remotes), inspected.chatcrystalKey, inspected.memorixKey, migrationState, existing?.created_at ?? now, now);
    ...
    getProject(id) {
        const row = this.db.prepare("SELECT * FROM projects WHERE id = ? AND active=1").get(id);
        return row ? this.projectFromRow(row) : null;
    }
    findProjectByAlias(kind, value) {
        const row = this.db
            .prepare(`
    ...
    listProjects() {
        const rows = this.db.prepare("SELECT * FROM projects WHERE active=1 ORDER BY updated_at DESC").all();
    ...
    projectFromRow(row) {
        ...
        return {
            ...
            remotes: json(row.remotes_json, []),
            chatcrystalKey: row.chatcrystal_key,
            memorixKey: row.memorix_key,
            sourceCoverage: Object.fromEntries(sourceRows.map((source) => [
                source.source,
    ...
    storeSession(session) {
        ...
        `)
                .run(sessionId, session.projectId, session.source, session.nativeId, session.sourcePath, session.sourceFingerprint, contentHash, session.cwd, session.startedAt, session.endedAt, session.messages.length, archivePath, JSON.stringify({
                __campSession: 1,
    ...
    getSession(sessionId, projectId) {
        ...
        const attachments = campEnvelope && Array.isArray(storedMetadata.attachments)
            ...
            : [];
        const ingestionCheckpoint = campEnvelope &&
    ...
    fingerprintFiles(projectId, relevantFiles) {
        ...
        return Object.fromEntries(relevantFiles.map((path) => {
            const absolute = isAbsolute(path) ? resolve(path) : resolve(project.rootPath, path);
            if (!isInsidePath(absolute, project.rootPath) || !existsSync(absolute))
                return [path, null];
            try {
    ...
    putEvidence(input) {
        ...
        const existing = this.db
            ...
            .get(input.projectId, input.kind, contentHash, input.state);
        if (existing)
            return this.evidenceFromRow(existing);
        const now = nowIso();
      ...
      `)
            .run(id, input.projectId, input.kind, input.state, input.title, input.content, input.confidence, input.sourceAgent, input.sourceSessionId, input.sourceUri, JSON.stringify(input.relevantFiles), input.commit, JSON.stringify(fileFingerprints), input.worktreeFingerprint, contentHash, now, now);
    ...
    evidenceFromRow(row) {
        return {
            ...
            relevantFiles: json(row.relevant_files_json, []),
            fileFingerprints: json(row.file_fingerprints_json, {}),
    ...
    latestHandoffPayload(projectId) {
        const row = this.db
            .prepare("SELECT payload_json FROM handoffs WHERE project_id=? ORDER BY created_at DESC LIMIT 1")
            .get(projectId);
        return row ? json(row.payload_json, null) : null;
    }
    refreshStaleness(projectId) {
        const project = this.getProject(projectId);
        if (!project)
            return 0;
        const rows = this.db
        ...
        const staleIds = [];
        for (const row of rows) {
            const files = json(row.relevant_files_json, []);
            const recorded = json(row.file_fingerprints_json, {});
            ...
            const stateSensitive = new Set(["progress", "verification", "handoff"]).has(row.kind);
            const gitChanged = stateSensitive &&
                ((row.commit_sha !== null && row.commit_sha !== commit) ||
                    (row.worktree_fingerprint !== null && row.worktree_fingerprint !== worktree));
            if (fileChanged || gitChanged)
    ...
    search(projectId, query, source = "all", limit = 20) {
    ...
    semanticCandidates(projectId, modelDigest, limit = 32) {
      ...
      `)
            .all(projectId, modelDigest, curatedBudget);
      ...
      `)
            .all(projectId, modelDigest, rawBudget);
    ...
    *semanticVectors(projectId, modelDigest, source = "all") {
    ...
    addQuarantine(input) {
      ...
      `)
            .run(id, input.projectId ?? null, input.source, input.sourcePath, input.nativeId ?? null, input.reason, JSON.stringify(input.metadata ?? {}), nowIso());
        return id;
    }
    listQuarantine(projectId) {
    ...
    isSourceAssigned(source, sourcePath, nativeId, projectId) {
        ...
        LIMIT 1
      `)
            .get(source, sourcePath, nativeId, projectId);
        return Boolean(row);
    }
    checkpoint(projectId, source, key) {
        const row = this.db
            .prepare("SELECT value FROM checkpoints WHERE project_id=? AND source=? AND key=?")
            .get(projectId, source, key);
        return row?.value ?? null;
    }
    setCheckpoint(projectId, source, key, value) {
        this.db
            .prepare(`
    ...
    enqueue(projectId, backend, action, payload, dedupeKey) {
        if (dedupeKey) {
            const existing = this.db
    ...
    beginBackendPurgeReceipt(projectId, backend, recordId, backendRecordId) {
        this.db
            .prepare(`
    ...
    completeBackendPurgeReceipt(projectId, backend, recordId) {
        this.db
            .prepare(`
    ...
    beginMigrationAudit(projectId, backend, contentHashes) {
    ...
    verifyMigration(projectId, backend, expectedHashes, completedHashes) {
      ...
      `)
            .run(verified ? "verified" : "pending", completed.length, manifestHash, verified ? nowIso() : null, verified ? null : "Completed payload hashes do not yet match the fallback manifest", projectId, backend);
        if (verified) {
            this.db
    ...
    migrationAudit(projectId, backend = "memorix") {
        return (this.db
            .prepare("SELECT * FROM migration_audits WHERE project_id=? AND backend=?")
            .get(projectId, backend) ?? null);
    }
    projectStatus(projectId) {
        const project = this.getProject(projectId);
        if (!project)
            throw new Error(`Unknown project: ${projectId}`);
        const scalar = (sql) => Number(this.db.prepare(sql).get(projectId).count);
        const sourceRows = this.db
    ...
    unregisterProject(projectId, purge) {
        if (purge) {
