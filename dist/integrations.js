...
export function detectClients() {
    ...
    const detections = [
        ...
        {
            name: "antigravity",
            installed: Boolean(commandPath("agy") ||
            ...
            surfaces: [
                ...(commandPath("agy") || existsSync(join(home, ".gemini", "antigravity-cli")) ? ["cli"] : []),
                ...(existsSync(join(home, ".gemini", "antigravity")) || existsSync(join(home, ".gemini", "antigravity-ide")) ? ["desktop"] : []),
            ],
        },
    ];
    return detections;
...
function cliInvocation() {
    ...
    if (existsSync(builtCli))
        return { command: process.execPath, args: [builtCli, "mcp"] };
    const sourceCli = resolve(root, "src", "cli.ts");
    const tsx = resolve(root, "node_modules", ".bin", "tsx");
    return { command: tsx, args: [sourceCli, "mcp"] };
}
function readManifest(store) {
    return readJsonFile(join(store.paths.home, "integrations.json"), {
        schemaVersion: 1,
        entries: [],
    });
}
function writeManifest(store, manifest) {
    atomicWrite(join(store.paths.home, "integrations.json"), `${JSON.stringify(manifest, null, 2)}\n`);
}
function trackInstall(store, entry) {
    const manifest = readManifest(store);
    const prior = manifest.entries.find((item) => item.client === entry.client && item.path === entry.path);
    manifest.entries = manifest.entries.filter((item) => !(item.client === entry.client && item.path === entry.path));
...
function installJson(store, client, path, invocation) {
    let current = {};
 ...
 */
function installJsonStringArray(store, client, path, fieldPath, values, detail, defaults = {}) {
    ...
    if (!field)
        throw new Error("Permission array path must not be empty");
    ...
    const existingValues = (priorValues ?? []).filter((value) => typeof value === "string");
...
function installClaudeHooks(store, path, invocation) {
    let current = {};
    if (existsSync(path)) {
        try {
            current = JSON.parse(readFileSync(path, "utf8"));
    ...
    const manifest = readManifest(store);
    const prior = manifest.entries.find((item) => item.client === "claude" && item.path === path && item.format === "json-hooks");
    const priorOwned = prior?.installedEntry && typeof prior.installedEntry === "object"
        ? prior.installedEntry
        : {};
    const priorEntries = priorOwned.__campHooks && typeof priorOwned.__campHooks === "object"
        ? priorOwned.__campHooks
        : priorOwned;
    const priorPermissionValues = Array.isArray(priorOwned.__campPermissionAllow)
        ? priorOwned.__campPermissionAllow.filter((value) => typeof value === "string")
        : [];
    ...
    if (current.permissions !== undefined &&
    ...
    const previousAllow = (permissions.allow ?? []).filter((value) => typeof value === "string");
...
function installOwnedJson(store, client, path, value) {
    const content = `${JSON.stringify(value, null, 2)}\n`;
    const manifest = readManifest(store);
    const prior = manifest.entries.find((item) => item.client === client && item.path === path && item.format === "owned-file");
...
function installJsonArrayEntry(store, client, path, field, key, keyValue, entry, defaults = {}) {
    let current = {};
    if (existsSync(path)) {
        try {
            current = JSON.parse(readFileSync(path, "utf8"));
    ...
    const entries = Array.isArray(current[field]) ? [...current[field]] : [];
    const index = entries.findIndex((item) => item && typeof item === "object" && item[key] === keyValue);
    if (index >= 0)
        entries[index] = entry;
    else
        entries.push(entry);
...
function installCodexHookPlugin(store, invocation) {
    const home = userHome();
    ...
    if (manifestResult.status === "error")
        return manifestResult.detail;
    const hooks = Object.fromEntries(["SessionStart", "UserPromptSubmit", "PostToolUse", "PreCompact", "Stop"].map((event) => [
        event,
        [
            {
    ...
    if (hooksResult.status === "error")
        return hooksResult.detail;
    const marketplace = installJsonArrayEntry(store, "codex", join(home, ".agents", "plugins", "marketplace.json"), "plugins", "name", "camp", {
        name: "camp",
    ...
    if (home !== homedir())
        return "CAMP Codex hook plugin written; runtime activation skipped in isolated mode";
    const codex = commandPath("codex");
    if (!codex)
        return "CAMP Codex hook plugin written; restart Codex after it is installed";
...
function installAntigravityHookPlugin(store, invocation) {
    const home = userHome();
...
function installCodexToml(store, path, invocation) {
...
export function installIntegrations(store) {
    const home = userHome();
    ...
    if (detected.get("cursor")?.installed) {
        ...
        if (cursor.surfaces.includes("ide")) {
            approvalResults.push(installJsonStringArray(store, "cursor", join(home, ".cursor", "permissions.json"), ["mcpAllowlist"], AUTO_APPROVED_CAMP_TOOLS.map((tool) => `camp:${tool}`), "Allowed only CAMP context receipt and verification tools in Cursor IDE"));
        }
        if (cursor.surfaces.includes("cli")) {
            approvalResults.push(installJsonStringArray(store, "cursor", join(home, ".cursor", "cli-config.json"), ["permissions", "allow"], AUTO_APPROVED_CAMP_TOOLS.map((tool) => `Mcp(camp:${tool})`), "Allowed only CAMP context receipt and verification tools in Cursor CLI", { version: 1, editor: { vimMode: false }, permissions: { allow: [], deny: [] } }));
        }
        result.detail = `${result.detail}; ${approvalResults.map((item) => item.detail).join("; ")}`;
        if (approvalResults.some((item) => item.status === "error"))
            result.status = "error";
        results.push(result);
    }
    else {
    ...
    if (detected.get("antigravity")?.installed) {
        ...
        const approval = installJsonStringArray(store, "antigravity", join(home, ".gemini", "antigravity-cli", "settings.json"), ["permissions", "allow"], AUTO_APPROVED_CAMP_TOOLS.map((tool) => `mcp(camp/${tool})`), "Allowed only CAMP context receipt and verification tools in Antigravity CLI");
...
export function removeIntegrations(store) {
    const manifest = readManifest(store);
    const results = [];
    const retained = [];
    const codexPluginFiles = manifest.entries.filter((item) => item.client === "codex" && item.format === "owned-file");
    const codexPluginFilesAreUnchanged = codexPluginFiles.length > 0 &&
        codexPluginFiles.every((item) => existsSync(item.path) && sha256(readFileSync(item.path)) === item.installedHash);
    ...
    for (const item of manifest.entries) {
        ...
        if (item.format === "toml-marker") {
            ...
            if (item.originalHash === null && !remaining)
                unlinkSync(item.path);
            else
                atomicWrite(item.path, `${remaining}\n`);
        ...
        if (item.format === "json-hooks") {
            ...
            try {
                ...
                const hooks = parsed.hooks && typeof parsed.hooks === "object" && !Array.isArray(parsed.hooks)
                    ? parsed.hooks
                    : {};
                const rawOwned = item.installedEntry;
                const owned = rawOwned.__campHooks && typeof rawOwned.__campHooks === "object"
                    ? rawOwned.__campHooks
                    : rawOwned;
                const permissionValues = Array.isArray(rawOwned.__campPermissionAllow)
                    ? rawOwned.__campPermissionAllow.filter((value) => typeof value === "string")
                    : [];
                let conflict = false;
                ...
                let permissions = parsed.permissions && typeof parsed.permissions === "object" && !Array.isArray(parsed.permissions)
                    ? { ...parsed.permissions }
                    : null;
        ...
        if (item.format === "json-string-array") {
            ...
            try {
                const parsed = JSON.parse(current);
                const owned = item.installedEntry;
                let owner = parsed;
                for (const field of owned.fieldPath.slice(0, -1)) {
                    const nested = owner ? owner[field] : undefined;
                    owner = nested && typeof nested === "object" && !Array.isArray(nested)
                        ? nested
                        : null;
                }
                const field = owned.fieldPath.at(-1);
                const entries = owner && field && Array.isArray(owner[field]) ? [...owner[field]] : null;
                ...
                const semanticallyEmpty = (value) => {
                    if (Array.isArray(value))
                        return value.length === 0;
                    if (!value || typeof value !== "object")
                        return false;
                    return Object.values(value).every(semanticallyEmpty);
                };
                if (item.originalHash === null && semanticallyEmpty(parsed))
                    unlinkSync(item.path);
                else
                    atomicWrite(item.path, `${JSON.stringify(parsed, null, 2)}\n`);
        ...
        if (item.format === "json-array") {
            try {
                const parsed = JSON.parse(current);
                const owned = item.installedEntry;
                const entries = Array.isArray(parsed[owned.field]) ? [...parsed[owned.field]] : [];
                const index = entries.findIndex((candidate) => JSON.stringify(candidate) === JSON.stringify(owned.entry));
                if (index < 0) {
                    retained.push(item);
                ...
                if (item.originalHash === null &&
                    entries.length === 0 &&
                    Object.keys(next).every((key) => key === owned.field || (key === "name" && next.name === "personal"))) {
                    unlinkSync(item.path);
                }
                else {
                    atomicWrite(item.path, `${JSON.stringify(next, null, 2)}\n`);
        ...
        try {
            ...
            delete servers.camp;
            const onlyEmptyServers = Object.entries(parsed).every(([key, value]) => key === "mcpServers" &&
                value !== null &&
                typeof value === "object" &&
                !Array.isArray(value) &&
                Object.keys(value).length === 0);
            if (item.originalHash === null && onlyEmptyServers)
                unlinkSync(item.path);
            else
                atomicWrite(item.path, `${JSON.stringify(parsed, null, 2)}\n`);
...
export function installLaunchAgent(store, activate = true) {
    const home = userHome();
...
export function removeLaunchAgent(store) {
...
function installSystemdUserService(store, activate) {
...
function windowsDaemonLauncher(store) {
    return join(store.paths.runtimeDir, "camp-daemon.cmd");
}
function installWindowsTask(store, activate) {
...
export function installUserService(store, activate = true) {
    const platform = hostPlatform();
    if (platform === "darwin")
        return installLaunchAgent(store, activate);
    if (platform === "windows")
        return installWindowsTask(store, activate);
    return installSystemdUserService(store, activate);
}
export function removeUserService(store) {
    const platform = hostPlatform();
    if (platform === "darwin")
        return removeLaunchAgent(store);
    if (platform === "windows") {
...
export function integrationHealth(store) {
    const manifest = readManifest(store);
    const entryHealthy = (entry) => {
        if (!existsSync(entry.path))
            return false;
        const content = readFileSync(entry.path, "utf8");
        if (entry.format === "toml-marker")
            return content.includes(String(entry.installedEntry));
        if (entry.format === "owned-file")
            return sha256(content) === entry.installedHash;
        try {
            ...
            if (entry.format === "json-hooks") {
                ...
                const rawOwned = entry.installedEntry;
                const ownedHooks = rawOwned.__campHooks && typeof rawOwned.__campHooks === "object"
                    ? rawOwned.__campHooks
                    : rawOwned;
                const hooksHealthy = Object.entries(ownedHooks).every(([event, owned]) => Array.isArray(hooks?.[event]) &&
                    hooks[event].some((candidate) => JSON.stringify(candidate) === JSON.stringify(owned)));
            ...
            if (entry.format === "json-string-array") {
                const owned = entry.installedEntry;
                if (!Array.isArray(owned.fieldPath) ||
                    !owned.fieldPath.every((value) => typeof value === "string") ||
                    !Array.isArray(owned.values) ||
                    !owned.values.every((value) => typeof value === "string")) {
                    return false;
                }
                let current = parsed;
            ...
            const owned = entry.installedEntry;
            return (Array.isArray(parsed[owned.field]) &&
                parsed[owned.field].some((candidate) => JSON.stringify(candidate) === JSON.stringify(owned.entry)));
        }
        catch {
            return false;
        }
    ...
    return detectClients().map((client) => {
        if (!client.installed)
            return { client: client.name, status: "degraded", detail: "Client is not installed" };
