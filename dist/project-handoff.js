...
function findSection(parsed, candidates) {
...
function listItems(lines, maximum = 8) {
    return lines
...
export function readProjectHandoff(rootPath) {
    for (const candidate of CANDIDATES) {
        ...
        unresolved.push(`The imported ${displayPath} snapshot is historical; inspect the current worktree and revalidate its test results before relying on it.`);
        const nextSteps = listItems(findSection(parsed, ["recommended next-agent sequence", "next steps", "recommended next"]));
        return {
            sourcePath,
            handoff: {
                goal,
...
//# sourceMappingURL=project-handoff.js.map