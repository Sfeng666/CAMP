...
export async function createAutomaticHandoff(store, project) {
    ...
    if (prior?.sourceSessions.includes(latest.id))
        return null;
...
export async function syncProject(store, project, onSource) {
    ...
    try {
        ...
        store.recordHealth(project.id, "backend:chatcrystal", chatcrystal.errors ? "degraded" : "ok", `total=${chatcrystal.total} errors=${chatcrystal.errors}`);
    }
    catch (error) {
