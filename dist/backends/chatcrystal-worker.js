...
function namespaceConversationId(source, sourceConversationId) {
...
function importItem(session, project, sourceInfo) {
    ...
    const idMap = new Map(session.messages.map((message) => [message.id, `${conversationId}:${message.id}`]));
    const parsed = {
        id: conversationId,
...
function insertMessages(db, item) {
  ...
  `);
    for (const [index, message] of item.parsed.messages.entries()) {
        insert.run(message.id, item.conversationId, message.parentUuid, message.type, message.role, message.content, message.hasToolUse ? 1 : 0, message.hasCode ? 1 : 0, message.thinking, message.timestamp, index);
    }
}
  ...
  `).get(item.source, item.sourceConversationId, item.conversationId);
    ...
    const transaction = db.transaction(() => {
      ...
      `).run(existing.id);
            ...
            const hasPreservedNote = Boolean(db.prepare("SELECT 1 FROM notes WHERE conversation_id=? LIMIT 1").get(existing.id));
            db.prepare(`
        UPDATE conversations SET
          slug=?, source=?, source_conversation_id=?, content_hash=?, parser_version=?,
          project_dir=?, project_name=?, cwd=?, git_branch=?, message_count=?,
      ...
      `).run(item.conversationId, ...metadataValues(item));
        ...
        db.prepare("INSERT INTO import_log(file_path, status, message) VALUES (?, 'success', ?)").run(item.meta.filePath, `${existing ? "Replaced" : "Imported"} ${item.parsed.messages.length} remote messages`);
    });
    transaction();
    return {
        source: item.source,
...
function merge(aggregate, item) {
    aggregate.total += 1;
    const status = String(item.status ?? "error");
    if (status === "imported")
        aggregate.imported += 1;
    else if (status === "replaced")
        aggregate.replaced += 1;
    else if (status === "skipped")
        aggregate.skipped += 1;
    else {
        aggregate.errors += 1;
        const id = String(item.conversationId ?? "");
        if (id && aggregate.errorIds.length < 100)
            aggregate.errorIds.push(id);
    }
    if (aggregate.items.length < 100)
        aggregate.items.push(item);
}
async function main() {
    const dataDir = process.env.DATA_DIR;
    if (!dataDir)
        throw new Error("ChatCrystal worker requires DATA_DIR");
    ...
    try {
        ...
        if (mode === "purge") {
            ...
            if (!projectId)
                throw new Error("ChatCrystal purge requires a project UUID");
            const result = db
        ...
        if (mode !== "ingest")
            throw new Error(`Unknown ChatCrystal worker mode: ${mode}`);
        const aggregate = {
            total: 0,
            imported: 0,
            replaced: 0,
        ...
        for await (const line of reader) {
            ...
            try {
                if (!value.archivePath)
                    throw new Error("ChatCrystal ingest descriptor is missing an archive path");
                const compressed = await readFile(value.archivePath);
                const session = JSON.parse((await gunzipAsync(compressed)).toString("utf8"));
                const item = importItem(session, value.project, value.sourceInfo);
                if (item)
                    merge(aggregate, ingestOne(db, item));
            }
            catch (error) {
                merge(aggregate, {
                    status: "error",
...
//# sourceMappingURL=chatcrystal-worker.js.map