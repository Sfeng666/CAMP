...
function openMemorixDatabase(store) {
...
function memorixProjectId(project) {
...
function eligibleForMemorix(record) {
...
export function matchMemorixObservations(records, observations) {
    ...
    for (const record of records) {
        const observation = observations.find((candidate) => candidate.title === record.title && candidate.narrative === record.content);
        if (observation)
            matched.push({ record, observation });
        else
            unmatched.push(record);
    }
    return {
        matched: [
            ...new Map(matched.map((pair) => [pair.observation.id, pair])).values(),
        ],
        unmatched,
    };
}
export function queueMemorix(store, project, record) {
    if (project.kind !== "git" || !eligibleForMemorix(record))
        return;
    store.enqueue(project.id, "memorix", "remember", record, stableId("memorix", project.id, "remember", record.id, record.contentHash));
}
function eligibleProjectRecords(store, projectId) {
    return store.listEvidence(projectId).filter(eligibleForMemorix);
}
export function prepareMemorixMigration(store, project) {
    ...
    store.beginMigrationAudit(project.id, "memorix", records.map((record) => record.contentHash));
...
export function finalizeMemorixMigration(store, project) {
    if (project.kind !== "git" || project.migrationState !== "pending-memorix")
        return false;
    const expected = eligibleProjectRecords(store, project.id).map((record) => record.contentHash);
    const rows = store.db
...
export async function flushMemorix(store, project, cooperate = async () => undefined, limit = 1) {
    const projectRows = () => store.pendingOutbox("memorix").filter((row) => row.project_id === project.id && row.action === "remember");
    let database;
    try {
            ...
            try {
                ...
                database.transaction(() => {
                    const existing = database
                        .prepare("SELECT id, title, narrative FROM observations WHERE projectId=? AND topicKey=? LIMIT 1")
                        .get(projectKey, topicKey);
                    if (existing) {
...
export function archiveMemorixProjectRecords(store, project) {
    if (project.kind !== "git") {
    ...
    const completedRows = store.db
       ...
       WHERE project_id=? AND backend='memorix' AND action='remember'
        ...
        .all(project.id);
    const records = [
        ...new Map(completedRows.flatMap((row) => {
            try {
                const record = JSON.parse(row.payload_json);
    ...
    try {
        const observations = database
            ...
            .all(projectKey);
        const receipts = new Map(initialReceipts.map((receipt) => [receipt.recordId, receipt]));
        ...
        for (const record of records) {
            ...
            const observation = observations.find((candidate) => String(candidate.id) === receipt.backendRecordId);
            if (!observation) {
        ...
        for (const pair of selection.matched) {
            store.beginBackendPurgeReceipt(project.id, "memorix", pair.record.id, String(pair.observation.id));
        }
        ...
        database
            .prepare(`UPDATE observations SET status='archived', updatedAt=?
        ...
        const deleted = database.transaction(() => {
            database
                .prepare(`DELETE FROM observation_code_refs WHERE projectId=? AND observationId IN (${placeholders})`)
                .run(projectKey, ...ids);
            return database
...
//# sourceMappingURL=memorix.js.map