...
function memorixEnvironment(dataDir) {
    return {
        ...process.env,
...
export function flushMemorix(store, project) {
    const binary = memorixBinary();
    if (!binary)
        return { completed: 0, failed: 0, unavailable: true, errors: ["memorix binary not found"] };
    let completed = 0;
    let failed = 0;
    const errors = [];
...
export function archiveMemorixProjectRecords(store, project) {
    ...
    try {
        ...
        const row = verify
            ...
            .get(...pairs.map((pair) => pair.observation.id));
