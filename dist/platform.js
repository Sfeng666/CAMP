...
export function isWsl(environment = process.env) {
...
export function hostPlatform(platform = process.platform, environment = process.env) {
    ...
    if (platform === "win32")
        return "windows";
    if (platform === "linux")
        return isWsl(environment) ? "wsl" : "linux";
    return "linux";
}
...
export function findCommand(command, environment = process.env, platform = hostPlatform(process.platform, environment)) {
    if (!command.trim())
        return null;
    if (command.includes("/") || command.includes("\\"))
        return existingPath(resolve(command));
...
export function commandString(command, args, platform = hostPlatform()) {
    if (platform === "windows") {
        const quote = (value) => `"${value.replaceAll(/(\\*)"/g, "$1$1\\\"").replaceAll(/(\\+)$/g, "$1$1")}"`;
        return [command, ...args].map(quote).join(" ");
    }
    const quote = (value) => `'${value.replaceAll("'", `'\\''`)}'`;
    return [command, ...args].map(quote).join(" ");
}
export function cursorUserDataDirectory(environment = process.env) {
    if (environment.CURSOR_DATA_DIR)
        return resolve(environment.CURSOR_DATA_DIR);
...
export function antigravityRoots(environment = process.env) {
    if (environment.ANTIGRAVITY_DATA_DIR)
        return [resolve(environment.ANTIGRAVITY_DATA_DIR)];
    const home = userHome(environment);
    return [
